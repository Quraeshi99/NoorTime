...
def get_request_location_and_method():
    """Helper to get location and method from request args or app defaults."""
    ...
    if req_lat is not None and req_lon is not None and req_method is not None:
        current_app.logger.info(f"Guest using query params for location: Lat={req_lat}, Lon={req_lon}, Method={req_method}")
        return req_lat, req_lon, req_method, req_city or "Custom Location"
    
    # Fallback to user defaults or app defaults
    if current_user.is_authenticated:
        return get_user_location_and_method(current_user)
    else:
...
def initial_prayer_data():
    try:
        ...
        lat, lon, method_key, city_name = get_request_location_and_method()
        ...
        if not user_prayer_settings_obj:
            user_prayer_settings_obj = UserSettings()
        ...
        data_to_send = {
            ...
            "apiTimesForDisplay": { 
                ...
                "CurrentTemperature": api_times_today.get("temperatureC", "N/A"),
                "WeatherDescription": api_times_today.get("weather_description", "N/A")
            },
            "dateInfo": {
            ...
            "userPreferences": {
                ...
                "homeLatitude": lat if current_user.is_authenticated else None,
...
def live_data():
    try:
        lat, lon, method_key, _ = get_request_location_and_method()
        ...
        tomorrow_fajr_details_for_service = {"azan": "N/A", "jamaat": "N/A"}
...
def update_user_settings():
    try:
        ...
        if 'preferences' in data:
            ...
            user_settings.adjust_timings_with_api_location = pref_data.get('adjust_offsets_on_location_change', user_settings.adjust_timings_with_api_location)
            user_settings.auto_update_api_location = pref_data.get('auto_update_location', user_settings.auto_update_api_location)
                ...
                if p_name in prayer_settings_data:
                    ...
                    if p_name == "jummah":
                        ...
                        if hasattr(user_settings, 'jummah_khutbah_start_time'):
                            user_settings.jummah_khutbah_start_time = p_data.get('fixed_khutbah', user_settings.jummah_khutbah_start_time)
